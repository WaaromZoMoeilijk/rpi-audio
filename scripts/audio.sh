...
# Unmount device
echo ; systemd-umount /dev/"$DEVICE" && echo -e "|" "${IGreen}Systemd-unmount - Done${Color_Off} |" >&2 || echo ; echo -e "|" "${IRed}Systemd-umount - Failed${Color_Off} |" >&2
echo ; umount /dev/"$DEVICE" && echo -e "|" "${IGreen}umount - Done${Color_Off} |" >&2 || echo ; echo -e "|" "${IGreen}Umount - Not mounted double check, done${Color_Off} |" >&2
...
if [ $? -eq 0 ]; then
    echo -e "|" "${IGreen}Systemd-unmount - Done${Color_Off} |" >&2
    # Remove folder after unmount
    rmdir "$MNTPTR" &&  echo ; echo -e "|" "${IGreen}$MNTPTR folder removed${Color_Off} |" >&2 || echo ; echo -e "|" "${IRed}$MNTPTR folder remove - Failed${Color_Off} |" >&2
else
