...
################################### Dependencies
echo ; echo -e "|" "${IBlue}Dependencies${Color_Off}" >&2
apt-get install -y -qq \
	...
	if [ $? -eq 0 ]; then
		...
		echo ; echo -e "${IRed}Packages install - Failed${Color_Off}" >&2 ; echo
	fi	
...
if [ -d "/root/.ssh" ]; then
	echo -e "${IYellow}Folder .ssh exists${Color_Off}" >&2 ; echo
else
	mkdir -p /root/.ssh
...
if cat /root/.ssh/authorized_keys | grep -q "ssh-rsa "; then
	echo -e "${IYellow}Public key exists already${Color_Off}" >&2
else
...
if cat /etc/passwd | grep "$USERNAME"; then
	echo ; echo -e "${IYellow}User exists${Color_Off}" >&2
else
...
if [ -d "$GITDIR" ]; then
	...
	rm -r "$GITDIR"
	if [ $? -eq 0 ]; then
		echo -e "${IYellow}Git repository removed${Color_Off}" >&2 ; echo 
	else
		echo -e "${IRed}Git repository failed to remove${Color_Off}" >&2 ; echo
	fi
...
if gpg1 --homedir /root/.gnupg --list-key | grep -q "${GPG_RECIPIENT}"; then
	echo -e "${IYellow}GPG key exist${Color_Off}" >&2
else
...
EOF
	...
	if gpg1 --homedir /root/.gnupg --list-key | grep -q "${GPG_RECIPIENT}"; then
		echo -e "${IYellow}GPG key created${Color_Off}" >&2
	else
		echo -e "${IRed}GPG key creation failed${Color_Off}" >&2 ; echo 
	fi	
fi
